...
for file in $(ls ${TOUCH_DIR}/mysql_backup_*_start*);do 
  ...
  if [ -f "$file" ];then
    ...
    editMetric "mysql_backup_start_timestamp" "Begining of the mysql backup in epoch" "gauge" "${database}"  ${timestamp_file} >> ${METRICS_FILE}
  fi
...
for file in $(ls ${TOUCH_DIR}/mysql_backup_*_end*);do 
  ...
  if [ -f "$file" ];then
    database=$(basename $file | awk -F "_" '{print $3}')
    timestamp_file=$(timestamp_file "$file")
    editMetric "mysql_backup_end_timestamp" "Begining of the mysql backup in epoch" "gauge" "${database}"  ${timestamp_file} >> ${METRICS_FILE}
  fi
